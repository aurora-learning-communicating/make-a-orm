...
public class Maximum<T extends Number & Comparable<T>> extends Aggregate<T> {
    public Maximum(@Nonnull Column<T> column) {
        super(column);
    }
    ...
    @Override
    public String alias() {
        return Quote.quoteAggregate("max", column);
    }
    ...
    @Override
    public String toSQL() {
        return "max(%s) as %s".formatted(Quote.quoteColumnStandalone(column), alias());
    }
}